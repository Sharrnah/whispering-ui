...
func BuildQwen3TTSSpecialSettings() fyne.CanvasObject {
	...
	capabilitiesLabel := widget.NewLabel(lang.L("Base checkpoints clone the selected voice sample; CustomVoice checkpoints use the built-in speaker list; VoiceDesign creates a voice from the instruction. The 0.6B CustomVoice checkpoint does not support instructions."))
	capabilitiesLabel.Wrapping = fyne.TextWrapWord
	streamingLabel := widget.NewLabel(lang.L("Live codec mode preserves one coherent generation. Parallel lookahead splits longer text at sentence/word boundaries and generates groups of independent segments in GPU batches (three per batch by default); this is much faster on high-end GPUs but can reset prosody at joins. It falls back to coherent codec mode when the text cannot be split. Adaptive buffering is designed to prevent gaps in either mode; fixed buffering starts sooner but can underrun."))
	streamingLabel.Wrapping = fyne.TextWrapWord
	...
	return container.New(layout.NewVBoxLayout(),
		...
		widget.NewAccordion(
			...
			widget.NewAccordionItem(lang.L("Generation"), container.New(layout.NewFormLayout(),
