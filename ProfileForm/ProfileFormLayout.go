...
func BuildAndRenderFullProfile(form *widget.Form, engine *FormEngine, deps FullFormDeps) *AllProfileControls {
    ...
    if audioSection.ApiSelect != nil && deps.OnAudioAPIChanged != nil { audioSection.ApiSelect.OnChanged = deps.OnAudioAPIChanged }
    if audioSection.InputSelect != nil && deps.OnAudioInputChanged != nil { audioSection.InputSelect.OnChanged = deps.OnAudioInputChanged }
    if audioSection.OutputSelect != nil && deps.OnAudioOutputChanged != nil { audioSection.OutputSelect.OnChanged = deps.OnAudioOutputChanged }
    ...
    energyBtn := widget.NewButtonWithIcon(lang.L("Autodetect"), theme.VolumeUpIcon(), func() {
        if deps.OnDetectEnergy == nil { return }
        selAPI, selInput := "", ""
        if audioSection.ApiSelect != nil && audioSection.ApiSelect.GetSelected() != nil { selAPI = audioSection.ApiSelect.GetSelected().Value }
        if audioSection.InputSelect != nil && audioSection.InputSelect.GetSelected() != nil { selInput = audioSection.InputSelect.GetSelected().Value }
        selInputText := ""
        if audioSection.InputSelect != nil && audioSection.InputSelect.GetSelected() != nil { selInputText = audioSection.InputSelect.GetSelected().Text }
        ...
        go func() {
            ...
            fyne.Do(func() {
                busy.Hide()
                if err != nil { dialog.ShowError(err, fyne.CurrentApp().Driver().AllWindows()[1]); return }
                energySlider.Max = SettingsMappings.EnergySliderMax
                if val >= energySlider.Max { energySlider.Max = val + 200 }
                energySlider.SetValue(val + 20)
                if deps.AfterDetectEnergy != nil { deps.AfterDetectEnergy() }
            })
        }()
    })
    ...
    if stt != nil {
        stt.TypeSelect.OnChanged = func(s CustomWidget.TextValueOption) { if engine.Coord != nil && !engine.Coord.InProgrammaticUpdate { engine.Coord.ApplySTTTypeChange(s.Value) } }
        stt.DeviceSelect.OnChanged = func(s CustomWidget.TextValueOption) {
            total := int64(0); if deps.TotalGPUMemory != nil { total = deps.TotalGPUMemory() }
            ...
            if engine.Coord != nil { prec := ""; if stt.PrecisionSelect.GetSelected() != nil { prec = stt.PrecisionSelect.GetSelected().Value }
                engine.Coord.EnsurePrecisionDeviceCompatibility(s.Value, prec); if !engine.Coord.InProgrammaticUpdate { engine.Coord.HandleMultiModalAllSync() } }
        }
        stt.PrecisionSelect.OnChanged = func(s CustomWidget.TextValueOption) {
            precisionType := Hardwareinfo.Float32
            switch s.Value { case "float32": precisionType = Hardwareinfo.Float32; case "float16": precisionType = Hardwareinfo.Float16; case "int32": precisionType = Hardwareinfo.Int32; case "int16": precisionType = Hardwareinfo.Int16; case "int8_float16", "int8", "int8_bfloat16": precisionType = Hardwareinfo.Int8; case "bfloat16": precisionType = Hardwareinfo.Float16; case "8bit": precisionType = Hardwareinfo.Bit8; case "4bit": precisionType = Hardwareinfo.Bit4 }
            total := int64(0); if deps.TotalGPUMemory != nil { total = deps.TotalGPUMemory() }
            AIModel := Hardwareinfo.ProfileAIModelOption{AIModel: "Whisper", Precision: precisionType}
            AIModel.CalculateMemoryConsumption(deps.CPUMemoryBar, deps.GPUMemoryBar, total)
            if engine.Coord != nil && stt.DeviceSelect.GetSelected() != nil { engine.Coord.EnsurePrecisionDeviceCompatibility(stt.DeviceSelect.GetSelected().Value, s.Value); if !engine.Coord.InProgrammaticUpdate { engine.Coord.HandleMultiModalAllSync() } }
        }
        stt.SizeSelect.OnChanged = func(s CustomWidget.TextValueOption) {
            ...
            AIModel.CalculateMemoryConsumption(deps.CPUMemoryBar, deps.GPUMemoryBar, total)
            if engine.Coord != nil && !engine.Coord.InProgrammaticUpdate { engine.Coord.HandleMultiModalAllSync() }
        }
    }
    ...
    if txt != nil {
        txt.DeviceSelect.OnChanged = func(s CustomWidget.TextValueOption) {
            if s.Value == "cuda" && deps.HasNvidiaGPU != nil && !deps.HasNvidiaGPU() && (engine.Coord == nil || !engine.Coord.InProgrammaticUpdate) { dialog.ShowInformation(lang.L("No NVIDIA Card found"), lang.L("No NVIDIA Card found. You might need to use CPU instead for it to work."), fyne.CurrentApp().Driver().AllWindows()[1]) }
            if txt.PrecisionSelect.GetSelected() != nil { if s.Value == "cpu" && (txt.PrecisionSelect.GetSelected().Value == "float16" || txt.PrecisionSelect.GetSelected().Value == "int8_float16" || txt.PrecisionSelect.GetSelected().Value == "bfloat16" || txt.PrecisionSelect.GetSelected().Value == "int8_bfloat16") { txt.PrecisionSelect.SetSelected("float32") }; if s.Value == "cuda" && txt.PrecisionSelect.GetSelected().Value == "int16" { txt.PrecisionSelect.SetSelected("float16") } }
            total := int64(0); if deps.TotalGPUMemory != nil { total = deps.TotalGPUMemory() }
            AIModel := Hardwareinfo.ProfileAIModelOption{AIModel: "TxtTranslator", Device: s.Value}
            AIModel.CalculateMemoryConsumption(deps.CPUMemoryBar, deps.GPUMemoryBar, total)
            if engine.Coord != nil { prec := ""; if txt.PrecisionSelect.GetSelected() != nil { prec = txt.PrecisionSelect.GetSelected().Value }; engine.Coord.EnsurePrecisionDeviceCompatibility(s.Value, prec); if !engine.Coord.InProgrammaticUpdate { engine.Coord.HandleMultiModalAllSync() } }
        }
        ...
        txt.TypeSelect.OnChanged = func(s CustomWidget.TextValueOption) { if engine.Coord != nil && !engine.Coord.InProgrammaticUpdate { engine.Coord.ApplyTXTTypeChange(s.Value) } }
    }
    ...
    if tts != nil {
        ...
        tts.TypeSelect.OnChanged = func(s CustomWidget.TextValueOption) { if engine.Coord != nil && !engine.Coord.InProgrammaticUpdate { engine.Coord.ApplyTTSTypeChange(s.Value) } }
    }

    ocr := builder.BuildOCRSection(engine)
    if ocr != nil {
        ocr.TypeSelect.OnChanged = func(s CustomWidget.TextValueOption) { if engine.Coord != nil && !engine.Coord.InProgrammaticUpdate { engine.Coord.ApplyOCRTypeChange(s.Value) } }
        ...
        ocr.PrecisionSelect.OnChanged = func(s CustomWidget.TextValueOption) {
            precisionType := Hardwareinfo.Float32
            switch s.Value { case "float32": precisionType = Hardwareinfo.Float32; case "float16", "bfloat16": precisionType = Hardwareinfo.Float16 }
            total := int64(0); if deps.TotalGPUMemory != nil { total = deps.TotalGPUMemory() }
    ...
    energySlider.OnChanged = func(v float64) { if v >= energySlider.Max { energySlider.Max += 10 }; energyState.SetText(fmt.Sprintf("%.0f", v)); if engine.Controls.PushToTalk.Text == "" && v == 0 { energySlider.SetValue(1); zeroEnergyInfo.Show() } }
    ...
    pauseSlider.OnChanged = func(v float64) { pauseState.SetText(fmt.Sprintf("%.1f", v)); if engine.Controls.PushToTalk.Text == "" && v == 0 { pauseSlider.SetValue(0.5); zeroPauseInfo.Show() } }
    ...
    engine.Controls.VadEnable.OnChanged = func(b bool) {
        if b { pauseSlider.Min = 0.0; phraseSlider.Min = 0.0; engine.Controls.VadConfidence.Show(); engine.Controls.VadRealtime.Show(); vadSection.PushToTalkBlock.Show() } else {
            engine.Controls.VadConfidence.Hide(); engine.Controls.VadOnFullClip.Hide(); engine.Controls.VadRealtime.Hide(); vadSection.PushToTalkBlock.Hide()
            if audioSection.ApiSelect != nil && audioSection.ApiSelect.Selected != "MME" && (engine.Coord == nil || engine.Coord.IsLoadingSettings == nil || !*engine.Coord.IsLoadingSettings) { dialog.ShowInformation(lang.L("Information"), lang.L("Disabled VAD is only supported with MME Audio API. Please make sure MME is selected as audio API. (Enabling VAD is highly recommended)"), fyne.CurrentApp().Driver().AllWindows()[1]) }
            if (pauseSlider.Value == 0 || phraseSlider.Value == 0) && (engine.Coord == nil || engine.Coord.IsLoadingSettings == nil || !*engine.Coord.IsLoadingSettings) { dialog.ShowInformation(lang.L("Information"), lang.L("You disabled VAD but have set the pause or phrase limit to 0. This is not supported. Setting Pause and Phrase limits to non-zero values."), fyne.CurrentApp().Driver().AllWindows()[1]); if pauseSlider.Value == 0 { pauseSlider.SetValue(1.2) }; if phraseSlider.Value == 0 { phraseSlider.SetValue(30) } }
            pauseSlider.Min = 0.1; phraseSlider.Min = 0.1
        }
    }

    var pushToTalkChanged bool
    engine.Controls.PushToTalk.OnChanged = func(s string) { if s != "" && (engine.Coord == nil || engine.Coord.IsLoadingSettings == nil || !*engine.Coord.IsLoadingSettings) { pushToTalkChanged = true } }
    engine.Controls.PushToTalk.OnFocusChanged = func(focusGained bool) {
        if !focusGained && pushToTalkChanged && engine.Controls.PushToTalk.Text != "" { dialog.NewConfirm(lang.L("Change speech trigger settings?"), lang.L("You did set a PushToTalk Button. Do you want to set settings to trigger with only a Button press?"), func(b bool) { if b { engine.Controls.Energy.SetValue(0); pauseSlider.SetValue(0); phraseSlider.SetValue(0) } }, fyne.CurrentApp().Driver().AllWindows()[1]).Show(); pushToTalkChanged = false }
    }
...
func AppendProfileLayout(form *widget.Form, controls any, rows []RowSpec, custom map[string]fyne.CanvasObject) {
    for _, row := range rows {
        if row.Spacer { form.Append("", layout.NewSpacer()); continue }
        if row.CustomKey != "" { if obj, ok := custom[row.CustomKey]; ok && obj != nil { appendWidgetToForm(form, row.Label, obj, row.Hint) }; continue }
        widgets := make([]fyne.CanvasObject, 0, len(row.ControlNames))
        for _, name := range row.ControlNames { if w := resolveControlByName(controls, name); w != nil { widgets = append(widgets, w) } }
        var content fyne.CanvasObject
        if row.Cols > 1 { content = container.NewGridWithColumns(row.Cols, widgets...) } else { if len(widgets) > 0 { content = widgets[0] } else { content = widget.NewLabel("") } }
        appendWidgetToForm(form, row.Label, content, row.Hint)
    }
}

func resolveControlByName(ctrls any, field string) fyne.CanvasObject {
    if ctrls == nil || field == "" { return nil }
    rv := reflect.ValueOf(ctrls); if rv.Kind() == reflect.Ptr { rv = rv.Elem() }
    f := rv.FieldByName(field); if !f.IsValid() || f.IsNil() { return nil }
    switch v := f.Interface().(type) {
    case *widget.Entry: return v
    case *widget.Check: return v
    case *widget.Slider: return v
    case *CustomWidget.TextValueSelect: return v
    case *CustomWidget.HotKeyEntry: return v
    default:
        if w, ok := f.Interface().(fyne.CanvasObject); ok { return w }
    }
    return nil
}
