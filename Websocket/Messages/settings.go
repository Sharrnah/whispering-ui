...
func (res TranslateSetting) Update() *TranslateSetting {
	...
	// Set TtsModelCombo
	if selected := ttsModelDisplayValue(res.Tts_model); selected != "" && len(Fields.Field.TtsModelCombo.Options) > 0 && Fields.Field.TtsModelCombo.Selected != selected {
		Fields.Field.TtsModelCombo.SetSelected(selected)
	}
	...
	if Fields.Field.TtsVoiceCombo.Selected != res.Tts_voice && hasSelectableTTSVoice(Fields.Field.TtsVoiceCombo.Options) {
